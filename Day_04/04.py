...
def part1():
    with open("input.txt") as f:
        accum = 0
        for line in f:
            winningNums = findwinners(line)
            ourNums = findournums(line)
            winctr = 0
            for num in ourNums:
                if num in winningNums:
                    winctr += 1
            # 1 point for first, x2 for each additional win
            if winctr > 0:
                accum += 2 ** (winctr - 1)
    print (accum)

def part2():
    # open the lines ahead of time
    scratchers = []
    with open("input.txt") as f:
        for line in f:
            scratchers.append(line)

    # make a list of how many of each copy we need to play, each starts at 1
    ticketCopies = []
    for ticket in scratchers:
        ticketCopies.append(1)

    gameCtr = 0
    for ticket in scratchers:
        # how many times we've played this ticket
        playCtr = 0

        winningNums = findwinners(ticket)
        ourNums = findournums(ticket)
        #number of winners for this ticket verion
        winctr = 0
        for num in ourNums:
            if num in winningNums:
                winctr += 1
        # increment next N tickets by 1 for each winner 
        while playCtr < ticketCopies[gameCtr]:
            extraCtr = 1
            while extraCtr <= winctr:
                ticketCopies[gameCtr + extraCtr] += 1
                extraCtr += 1
            playCtr += 1
        # move onto next ticket number
        gameCtr += 1

    # all games played, sum number of tickets
    print (sum(ticketCopies))


def findwinners(line):
    firstBreak = line.find(":")
    secondBreak = line.find("|")
    winningSection = line[firstBreak + 1:secondBreak].split()
    numList = [int(match) for match in winningSection]
    return  numList

def findournums(line):
    firstBreak = line.find("|")
    ourSection = line[firstBreak + 1:].split()
